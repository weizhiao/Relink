...
| --- | --- | --- | --- |
| `x86_64` | 支持，主要验证路径 | 支持：布局 pass、section placement、hot code 聚集和大页 arena | 支持，需开启 `object` feature |
| `x86` | 支持 | 支持基础依赖规划；复杂 section 重排和 hot code 聚集待实现 | 不支持，待实现 |
| `aarch64` | 支持 | 支持基础依赖规划；复杂 section 重排和 hot code 聚集待实现 | 不支持，待实现 |
| `arm` | 支持 | 支持基础依赖规划；复杂 section 重排和 hot code 聚集待实现 | 不支持，待实现 |
| `riscv64` | 支持 | 支持基础依赖规划；复杂 section 重排和 hot code 聚集待实现 | 不支持，待实现 |
| `riscv32` | 支持 | 支持基础依赖规划；复杂 section 重排和 hot code 聚集待实现 | 不支持，待实现 |
| `loongarch64` | 支持 | 支持基础依赖规划；复杂 section 重排和 hot code 聚集待实现 | 不支持，待实现 |

动态库与可执行文件加载是当前所有内置架构后端的主要支持路径。复杂 section 重排修复和 `.o` / `ET_REL` 支持目前主要围绕 `x86_64` 的重定位实现展开；其他架构仍待实现。
