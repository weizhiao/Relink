...
| 可替换映射后端 | 接入不同平台的 mmap 后端，由映射区域定义读写/借用行为、权限模型、页大小或大页策略 |
...
| 映射策略 | ✅ mmap 后端、映射区域访问模型、页大小和大页策略可替换 | ❌ |
...
| 自定义映射环境 | `Loader::with_mmap(mapper)` / `with_page_size()` | 需要接入返回 `MappedRegion`/`MmapResult` 的 mmap 后端，并由映射区域控制内存访问、权限模型、页大小或大页策略 |
