...
| ---------------- | -------------------------------------------- | -------------------------- |
| 内存加载         | ✅ 路径 / 内存缓冲区 / 已解析 ELF             | △ 通常依赖文件路径         |
| `ET_REL` 加载    | ✅ feature-gated                              | ❌ 不支持                   |
| 链接前规划       | ✅ 依赖 / section / 映射策略可预先规划        | ❌ 不支持                   |
| 动态链接期优化   | ✅ section 重排 / hot code 聚集 / 自定义 pass | ❌ 不支持                   |
| 大页与映射策略   | ✅ 映射后端可替换                             | △ 通常不可控               |
| 依赖与符号策略   | ✅ 依赖图 / 作用域 / 查找 / 拦截可控          | △ 通常不可控               |
| 链接上下文隔离   | ✅ 多个 `LinkContext` 隔离依赖图和符号作用域  | △ 通常依赖进程级链接器状态 |
| 异构加载         | ✅ 可扫描和装载不同 ELF / ABI / 目标架构镜像  | △ 依赖宿主动态链接器       |
| 符号生命周期安全 | ✅ 类型化符号绑定已加载镜像生命周期           | ❌ 不支持                   |
