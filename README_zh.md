...
| --- | --- | --- |
| 内存加载 | ✅ 路径 / 内存缓冲区 / 已解析 ELF | ❌ 不支持 |
| `ET_REL` 加载 | ✅ feature-gated | ❌ 不支持 |
| 链接前规划 | ✅ 依赖 / section / 映射策略可预先规划 | ❌ 不支持 |
| 动态链接期优化 | ✅ section 重排 / hot code 聚集 / 自定义 pass | ❌ 不支持 |
| 映射策略 | ✅ mmap 后端、页大小和大页策略可替换 | ❌ 不支持 |
| 依赖与符号策略 | ✅ 依赖图 / 作用域 / 查找 / 拦截可控 | ❌ 不支持 |
| 上下文隔离 | ✅ 多个 `LinkContext` 隔离依赖图和符号作用域 | ❌ 不支持 |
| 异构加载 | ✅ 可扫描和装载不同 ELF / ABI / 目标架构镜像 | ❌ 不支持 |
...
| 映射前布局优化 | `Linker::load_scan_first()`、`map_pipeline()`，见 `cargo run --example load_scan_first` |
