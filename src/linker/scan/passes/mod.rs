...
use crate::{
    ...
    linker::ModuleKey, relocation::RelocationArch, tls::TlsResolver,
};
...
impl<'a, S, Arch, Tls> LinkPassPlan<'a, S, Arch, Tls>
where
    S: PassScopeMode,
    Arch: RelocationArch,
    Tls: TlsResolver<Arch>,
{
    #[inline]
    fn new(plan: &'a mut LinkPlan<Arch, Tls>) -> Self {
        Self {
            plan,
            scope: PhantomData,
        }
    ...
    #[inline]
    pub fn module(&self, key: &ModuleKey) -> Option<Module<'a, S>> {
...
pub trait LinkPass<S = AnyPass, Arch: RelocationArch = NativeArch, Tls: TlsResolver<Arch> = ()>
where
    S: PassScopeMode,
{
    /// Executes the pass over the current plan.
    fn run(&mut self, plan: &mut LinkPassPlan<'_, S, Arch, Tls>) -> Result<()>;
}

type PipelinePass<'a, Arch, Tls> = Box<dyn FnMut(&mut LinkPlan<Arch, Tls>) -> Result<()> + 'a>;
...
pub struct LinkPipeline<'a, Arch: RelocationArch = NativeArch, Tls: TlsResolver<Arch> = ()> {
    passes: Vec<PipelinePass<'a, Arch, Tls>>,
}

impl<'a, Arch, Tls> Default for LinkPipeline<'a, Arch, Tls>
where
    Arch: RelocationArch,
    Tls: TlsResolver<Arch>,
{
...
impl<'a, Arch, Tls> LinkPipeline<'a, Arch, Tls>
where
    Arch: RelocationArch,
    Tls: TlsResolver<Arch>,
{
    ...
    where
        S: PassScopeMode + 'a,
        P: LinkPass<S, Arch, Tls> + 'a,
    {
        self.passes.push(Box::new(move |plan| {
            let mut scoped = LinkPassPlan::<S, Arch, Tls>::new(plan);
            pass.run(&mut scoped)
        }));
        self
    }
    ...
    pub(crate) fn run(&mut self, plan: &mut LinkPlan<Arch, Tls>) -> Result<()> {
